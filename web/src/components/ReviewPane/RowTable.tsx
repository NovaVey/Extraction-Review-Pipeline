...
export function RowTable({ rows, onAcceptRow, onCorrectRow }: RowTableProps) {
  if (rows.length === 0) return <p className="text-sm text-[#6B7280]">This table has no rows.</p>;
      ...
      <table className="w-full min-w-max text-sm">
        <thead>
          <tr className="bg-[#F9FAFB] text-left">
            {columns.map((col) => (
              <th key={col.key} className="whitespace-nowrap border-b border-[#E5E7EB] px-2 py-1.5 font-medium text-[#101114]">
                {col.label}
              </th>
            ))}
...
function RowTableRow({ row, columns, onAcceptRow, onCorrectRow }: RowTableRowProps) {
  ...
  async function handleCellKeyDown(event: KeyboardEvent<HTMLInputElement>, columnKey: string) {
    if (event.key === 'Enter') {
      ...
      if (!canAccept || values[columnKey] === originals[columnKey] || pending) return;
      setPending(true);
      setError(null);
      ...
      {columns.map((col) => (
        <td key={col.key} className="border-b border-[#E5E7EB] px-2 py-1 align-top">
          <input
            ...
            className="w-full min-w-24 rounded border border-[#E5E7EB] bg-white px-1.5 py-1 text-sm disabled:opacity-50"
          />
        </td>
      ))}
        ...
        <div className="flex flex-col items-start gap-1">
          {canAccept ? (
            <button
              ...
              disabled={pending}
              className="rounded border border-[#101114] bg-[#101114] px-2 py-1 text-xs font-medium text-white disabled:opacity-50"
            >
              Accept row
            </button>
          ) : (
